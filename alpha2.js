...
document.getElementById('searchForm').addEventListener('submit', async function (event) {
    ...
    // Construct the search URL with filtered fields
    const fields = 'image_id,title,artist_display,place_of_origin,thumbnail.alt_text,short_description,date_display,id,category_titles';
    const searchUrl = `https://api.artic.edu/api/v1/artworks/search?q=${encodeURIComponent(searchText)}&fields=${encodeURIComponent(fields)}`;

    // Call the function to fetch data based on the search URL
    await getData(searchUrl);
});

// Function to fetch data from the API
async function getSearch(urll) {
...
function htmlBuild() {
    ...
    artworks.forEach(artwork => {
        ...
        divArt.classList.add('artwork', 'px-4', 'm-5', 'py-5', 'col-md-3', 'col-lg-3', 'col-12', 'img-fluid',);
